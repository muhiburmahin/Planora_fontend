...
interface JoinEventModalProps {
    event: Event;
    existingParticipationId?: string;
    onClose: () => void;
    onSuccess: () => void;
}
...
// ─── Main Modal ───────────────────────────────────────────────────────────────
export default function JoinEventModal({ event, existingParticipationId, onClose, onSuccess }: JoinEventModalProps) {
    const isFree = event.registrationFee === 0;
    const [step, setStep] = useState<"confirm" | "payment" | "success" | "error">(existingParticipationId ? "payment" : "confirm");
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const [participationId, setParticipationId] = useState(existingParticipationId || "");
